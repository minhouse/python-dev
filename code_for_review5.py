...
excel_dir = 'C:/Users/TokuharM.AP/mywork/'
writer = pd.ExcelWriter('%sReading_Performance_Report_%s.xlsx' % (excel_dir, today_date))
...
class DistrictPerformance:
    def __init__(self, cr_list, attr):
        ...
        self.cr_rank = cr_list.pivot_table(values=['meter_status'], index=['name'], columns=['abc_rank'], aggfunc='count')
        ...
        self.district_cr_rank = self.district_meter.pivot_table(values=['meter_status'], index=['name'], columns=['abc_rank'], aggfunc='count')
        # self.cr_rank.columns = self.cr_rank.columns.droplevel()
        self.district_cr_rank = self.district_cr_rank.loc[:, ['P', 'A', 'B', 'C', 'D', 'E', 'F']]
        self.district_cr_rank = self.district_cr_rank.fillna(0)
    ...
    def write_to_excel(self, cr_list, sheetname, sheetname2, sheetname3, sheetname4, sheetname5, todaydate, excel_dir, writer):
        cr_list.to_excel(writer, sheetname, index=False)
        self.workbook = writer.book
        self.worksheet = writer.sheets[sheetname]
        self.cr_perf.to_excel(writer, sheetname2)
        self.worksheet = writer.sheets[sheetname2]
        self.worksheet.set_column(0, 0, 27)
        self.worksheet.set_column(1, 2, 16)
        self.cr_rank.to_excel(writer, sheetname3)
        self.worksheet = writer.sheets[sheetname3]
        self.worksheet.set_column(0, 0, 11)
        self.worksheet.set_column(1, 7, 9)
        self.region_perf.to_excel(writer, sheetname4)
        self.worksheet = writer.sheets[sheetname4]
        self.worksheet.set_column(0, 0, 27)
        self.worksheet.set_column(1, 2, 16)
        self.area_perf.to_excel(writer, sheetname5)
        self.worksheet = writer.sheets[sheetname5]
        self.worksheet.set_column(0, 0, 37)
        self.worksheet.set_column(1, 7, 9)

    def write_to_excel_district(self, todaydate, excel_dir, writer):
        self.district_cr_rank.to_excel(writer, "{} CR Rank Perf".format(self.name))
        self.worksheet = writer.sheets["{} CR Rank Perf".format(self.name)]
        self.worksheet.set_column(0, 0, 11)
        self.worksheet.set_column(1, 7, 9)
...
class CollectorPerformance(DistrictPerformance):
    ...
    def write_to_excel(self, sheetname, todaydate, excel_dir, writer):
        self.cr_perf.to_excel(writer, sheetname, index=False)
        self.workbook = writer.book
        self.worksheet = writer.sheets[sheetname]
        self.cr_perf.to_excel(writer, sheetname)
        self.worksheet = writer.sheets[sheetname]
        self.worksheet.set_column(0, 0, 27)
        self.worksheet.set_column(1, 2, 16)
...
class KeyPerformanceIndicator:
    def __init__(self, Total_AllMeter_Count, Total_LDAMeter_Count, onlycell_meter, Total_HighRiseMeter_Count,
                 Total_VillageMeter_Count, unknownbuilding_Count, Total_CellMeter_Count, cr_list):
...
class AllMetersCount:
    def __init__(self, Total_ALLCellMeter_Count, Total_HighRiseMeter_Count, Total_VillageMeter_Count, 
                 Total_CellMeter_Count, Total_LDAMeter_Count, unlocated_meter_Count):
...
class AllLPIntervalPushSuccessRate:
    def __init__(self, Total_HighRiseMeter_Count, Total_VillageMeter_Count, Total_CellMeter_Count, 
                 Total_LDAMeter_Count, cr_highrise, cr_village, onlycell_meter, LDA_meter):
...
class AllDayendPushSuccessRate:
    def __init__(self, Total_HighRiseMeter_Count, cr_highrise, Total_VillageMeter_Count, cr_village, 
                 Total_CellMeter_Count, Total_LDAMeter_Count, onlycell_meter, LDA_meter):
...
class AllLpPushCountPerformance:
    def __init__(self, cr_list, Total_HighRiseMeter_Count, Total_VillageMeter_Count, unknownbuilding_Count, 
                 Total_LDAMeter_Count, cr_highrise, cr_village, Total_CellMeter_Count, onlycell_meter, LDA_meter):
        self.Expected_AllMeter_Total_LP_Count = (((Total_HighRiseMeter_Count + Total_VillageMeter_Count + unknownbuilding_Count) - Total_LDAMeter_Count) * 48) + (Total_LDAMeter_Count * 144)
        self.AllMeter_Total_LP_Count = cr_list['NoOfIntervals'].sum()
        self.Expected_HighRiseMeter_Total_LP_Count = Total_HighRiseMeter_Count * 48
        self.HighRiseMeter_Total_LP_Count = cr_highrise['NoOfIntervals'].sum()
...
class MeterTypeCompositionRate:
    def __init__(self, Total_AllMeter_Count, Normal_Meter_Count, SecConfig_Meter_Count, Config_Meter_Count, 
                 Discovered_Meter_Count, Failed_Meter_Count, Lost_Meter_Count, Total_ALLCellMeter_Count, Total_HighRiseMeter_Count, 
                 Total_VillageMeter_Count, unlocated_meter_Count, Total_LDAMeter_Count, Total_CellMeter_Count):
...
class FirmwarePerformance():
    ...
    def write_to_excel(self, sheetname, todaydate, excel_dir, writer):
        self.fw_perf.to_excel(writer, sheetname, index=False)
        self.workbook = writer.book
        self.worksheet = writer.sheets[sheetname]
        self.fw_perf.to_excel(writer, sheetname)
        self.worksheet = writer.sheets[sheetname]
        self.worksheet.set_column(0, 0, 27)
        self.worksheet.set_column(1, 2, 16)
...
class ToDataFrame():
    ...
    def write_to_excel(self, sheetname, todaydate, excel_dir, writer):
        self.performance.to_excel(writer, sheetname, index=False)
        self.workbook = writer.book
        self.worksheet = writer.sheets[sheetname]
        self.performance.to_excel(writer, sheetname)
...
def main():
    ...
    df_performance.write_to_excel('Performance Report', today_date, excel_dir, writer)
    df_performance = df_performance.output_dataframe()
    print(df_performance)
    ToExcel = DistrictPerformance(cr_list, 'Analyzed Individual Meters')
    ToExcel.write_to_excel(cr_list, 'Analyzed Individual Meters', 'CR Performance', 'CR ABC Rank', 'Region Performance', 'Area ABC Rank', today_date, excel_dir, writer)
    firmwareperformance = FirmwarePerformance(cr_list)
    firmwareperformance.write_to_excel('FW Performance', today_date, excel_dir, writer) 
    ...
    for instance in write_excel_list:
        instance.write_to_excel_district(today_date, excel_dir, writer)
    writer.save()
