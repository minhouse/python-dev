...
def main():
    ...
    ToExcel.write_to_excel(cr_list, 'Analyzed Individual Meters', 'CR Performance', 'CR ABC Rank', 'Region Performance', 'Area Performance', today_date, excel_dir, writer)

    #以下４つの出力処理はエクセルに出力されない
    #出力処理２
    firmwareperformance = FirmwarePerformance(cr_list)
    firmwareperformance = firmwareperformance.output_fw_stats()
    firmwareperformance2 = FirmwarePerformance(cr_list)
    firmwareperformance2.write_to_excel('FW Performance', today_date, excel_dir, writer) 
    #出力処理３
    df_performance = ToDataFrame('Performance Result', new_dict)
    df_performance = df_performance.output_dataframe()
    df_performance2 = ToDataFrame('Performance Result', new_dict)
    df_performance2.write_to_excel('Performance Report', today_date, excel_dir, writer)
    #出力処理４
    cr_performance = CollectorPerformance(cr_list)
    cr_performance = cr_performance.get_collector_statistics()
    cr_performance2 = CollectorPerformance(cr_list)
    cr_performance2.write_to_excel('test', today_date, excel_dir, writer)
    #出力処理５
    write_excel_list = [
